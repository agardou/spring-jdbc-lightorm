 ...
 */
public abstract class AbstractAnnotatedBeanDao<T> {
	@Autowired
	...
	public AbstractAnnotatedBeanDao() {
	...
	@PostConstruct
	private void postConstruct() throws DaoException {
	...
	public T createOrUpdate(T obj) throws DaoException {
	...
	public T create(T obj) throws DaoException {
		...
		if (!beanMappingDescriptor.isIdAutoIncremented()) {
			for (Object value : fieldIdParams.values()) {
				if (value == null)
					throw new DaoException("Impossible de créer un objet sans clé primaire renseignée");
			}
		}
	...
	private void createWithKeyHolder(T obj, String sql, Map<String, Object> allParams) throws DaoException {
		...
		if (fieldIdDescriptor.getField().getType().equals(Integer.class))
			...
			throw new DaoException("Auto-incremented field [" + fieldIdDescriptor.getField().getName() + "] of type [" + fieldIdDescriptor.getField().getType().getCanonicalName() + "]not managed ");
	}
	...
	public void update(T obj) throws DaoException {
		...
		for (Object value : fieldIdParams.values()) {
			if (value == null)
				throw new DaoException("Impossible de mettre à jour un objet sans clé primaire renseignée");
		}
	...
	public List<T> create(List<T> objList) throws DaoException {
		for (T obj : objList)
			create(obj);
		return objList;
	}

	public List<T> update(List<T> objList) throws DaoException {
		for (T obj : objList)
			update(obj);
		return objList;
	}

	public List<T> createOrUpdate(List<T> objList) throws DaoException {
		for (T obj : objList)
			createOrUpdate(obj);
		return objList;
	}
	...
	public T findById(Object id) throws DaoException {
		if (beanMappingDescriptor.getFieldIdMappingDescriptors().size() != 1)
			throw new DaoException("Le nombre de champs ID ne correspondent pas [" + this.getTableClass().getCanonicalName() + "]");
	 ...
	 * @throws DaoException
	 */
	public T findById(Map<String, Object> ids) throws DaoException {
		...
		for (FieldMappingDescriptor fieldId : beanMappingDescriptor.getFieldIdMappingDescriptors()) {
			...
			if (!ids.keySet().contains(fieldId.getColumnName()))
				throw new DaoException("Le champs ID [" + fieldId.getColumnName() + "] recherché ne correspondent pas [" + this.getTableClass().getCanonicalName() + "]");
	...
	public void delete(T obj) throws DaoException {
		...
		for (Object value : params.values()) {
			if (value == null)
				throw new DaoException("Impossible de supprimer un objet sans clé primaire renseignée");
		}
	...
	protected Map<String, Object> extractIdParams(T obj) throws DaoException {
		return extractParams(obj, beanMappingDescriptor.getFieldIdMappingDescriptors());
	}

	protected Map<String, Object> extractNonIdParams(T obj) throws DaoException {
	 ...
	 * @throws DaoException
	 */
	private Map<String, Object> extractParams(T obj, List<FieldMappingDescriptor> fieldMappingDescriptors) throws DaoException {
	...
	public void loadLazyProperties(T obj, String[] properties) throws DaoException {
		...
		for (String property : properties) {
			...
			try {
				...
				if (lazyLoading == null)
					throw new DaoException("LazyLoading annotation not found on field [" + property + "], Table Class [" + getTableClass().getName() + "]");

				PropertyDescriptor pdWithKey = BeanUtils.getPropertyDescriptor(tableClass, lazyLoading.value());

				AbstractAnnotatedBeanDao<?> dao = RegisteredDao.getDao(fieldToLoad.getType());
				if (dao == null)
					throw new DaoException("Dao not found for Table Class [" + getTableClass().getName() + "]");
				...
				throw new DaoException("Field [" + property + "] does not exist on Table Class [" + getTableClass().getName() + "]");
